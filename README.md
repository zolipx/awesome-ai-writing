...
仓库每天自动检查现有 GitHub 项目的可访问性、许可证、维护时间和社区信号，并搜索新的候选项目。自动任务只生成 [`reports/latest.md`](reports/latest.md) 证据报告和审核 PR，不会直接改写本目录，也不会自动认定候选项目值得收录。所有新增、删除和描述修改仍需维护者核验来源后合并。

自动化边界、候选分级和运行方式见 [`AUTOMATION.md`](AUTOMATION.md)。
